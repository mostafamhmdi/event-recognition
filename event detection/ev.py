...
class QwenEventVerifier:
    ...
    def _build_prompt(self, messages):
        """
        Builds the chat-template prompt and appends a forced opening brace
        ("{") at the very end, so the model is nudged straight into JSON
        instead of chit-chat before it.

        This tries with ALL of `messages` first; if the resulting prompt
        (system + user + forced "{") doesn't fit under max_input_tokens,
        the LAST message is dropped and it tries again, repeating until it
        fits (or only one message is left).

        This is the actual fix for the parsing failures: the old code
        tokenized the full prompt with `truncation=True,
        max_length=max_input_tokens`, and HF tokenizers truncate from the
        END by default - on long/many messages this silently chopped off
        the forced "{" itself before the model ever saw it, so the model
        just kept writing plain text (continuing the last message's
        sentence) instead of JSON. That's exactly the "Expecting property
        name enclosed in double quotes" / "Expecting value" errors and the
        garbled continuation text. Trimming whole messages up front instead
        of letting the tokenizer truncate mid-prompt guarantees the forced
        "{" always survives intact.

        Returns (prompt_text, messages_actually_used).
        """
...
۴. **تمرکز و یکتایی:** شخصیت‌ها، مکان‌ها و زمان‌های اصلی در پیام‌ها هم‌پوشانی کامل داشته باشند. (پیام‌ها صرفاً تحلیل یا گمانه‌زنی پراکنده نباشند).
...
۴. **کلاستر کثیف:** وجود چندین خبر یا موضوع مختلف و نامرتبط در یک خوشه.
