...
#     def __init__(self, text_col: str = '0', future_date_col: str = 'future_date', loc_col: str = 'location_entities'):
#         self.text_col = text_col
#         self.future_date_col = future_date_col
#         self.loc_col = loc_col

#     def cluster_Events(self, df: pd.DataFrame) -> pd.DataFrame:
#         print("\n🔍 شروع خوشه‌بندی بر اساس تاریخ آینده و مکان‌های مشترک (روی کل داده‌ها)...")
#         working_df = df.copy()
...
#             working_df[self.loc_col].notna() & 
#             (working_df[self.loc_col] != 'None')
#         )
...
#         print(f"تعداد رکوردهای معتبر برای خوشه‌بندی (دارای تاریخ و مکان): {len(filtered_df)}")
        
#         # گروه‌بندی بر اساس تاریخ آینده
#         grouped_by_future = filtered_df.groupby(self.future_date_col)
#         cluster_global_id = 1

#         for future_date, group_df in grouped_by_future:
#             # ساخت گراف برای پیدا کردن اشتراکات مکانی در این تاریخ خاص
#             G = nx.Graph()
            
#             # اضافه کردن گره‌ها (توییت‌ها)
#             for idx, row in group_df.iterrows():
#                 # تبدیل رشته‌ی مکان‌ها به یک Set برای مقایسه راحت‌تر
#                 locs_str = str(row[self.loc_col]).replace('،', ',')
#                 locs_set = set([loc.strip() for loc in locs_str.split(',') if loc.strip()])
#                 G.add_node(idx, locations=locs_set, data=row)
            
#             # پیدا کردن اشتراکات و کشیدن یال بین گره‌ها
#             nodes = list(G.nodes(data=True))
#             for i in range(len(nodes)):
#                 for j in range(i + 1, len(nodes)):
#                     node1_idx, node1_data = nodes[i]
#                     node2_idx, node2_data = nodes[j]
                    
#                     if node1_data['locations'].intersection(node2_data['locations']):
#                         G.add_edge(node1_idx, node2_idx)
                        
#             # استخراج خوشه‌ها از طریق پیدا کردن اجزای متصل گراف
#             clusters = list(nx.connected_components(G))
            
#             # ثبت شناسه‌ی خوشه در دیتافریم اصلی
#             for local_cluster_id, node_indices in enumerate(clusters):
#                 # برای خوانایی بهتر، آیدی خوشه شامل شماره و تاریخ است
#                 safe_date = future_date.replace('/', '')
#                 current_cluster_name = f"Cluster_{cluster_global_id}_{safe_date}"
                
#                 # اعمال شناسه خوشه به سطرهای مربوطه در دیتافریم کل
#                 working_df.loc[list(node_indices), 'cluster_id'] = current_cluster_name
                
#                 print(f"  * خوشه یافت شد! تاریخ: {future_date} | تعداد توییت: {len(node_indices)} | شناسه: {current_cluster_name}")
#                 cluster_global_id += 1
...
# def process_clusters(df: pd.DataFrame, text_col: str = '0') -> pd.DataFrame:
